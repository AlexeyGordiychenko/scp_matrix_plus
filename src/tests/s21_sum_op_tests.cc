...
TEST(SumMatrixOP1, 1) {
  S21Matrix m1({1, 1, 58701877.20069275});
  S21Matrix m2({1, 1, 80866809.58969878});
  S21Matrix m1_res({1, 1, 139568686.79039153});

  EXPECT_TRUE(m1 + m2 == m1_res);
}

TEST(SumMatrixOP1, 2) {
  S21Matrix m1({3, 3, -0.5694129202517216, 0.6200879013819285,
                -0.727134719033829, -6648787.898751973, 3241014937.954668,
                1.5304569679582591, -285088.00550373725, -4255.438909984965,
                -207006074.77125922});
  S21Matrix m2({3, 3, -173876851.07449624, 1500740204.5287642,
                8751.469380971424, 663472263.5896968, 328106966.1934105,
                5803312.304089934, -1375219943.7604141, 262397652.0142747,
                6327178.300569486});
  S21Matrix m1_res({3, 3, -173876851.64390916, 1500740205.148852,
...
TEST(SumMatrixOP1, 3) {
  S21Matrix m1({2, 6, -897076224.3607916, -0.5977138230531812,
  ...
  S21Matrix m2({2, 6, 0.5254873381838764, 0.2723917020943324, 68772043.1870324,
  ...
  S21Matrix m1_res({2, 6, -897076223.8353043, -0.3253221209588488,
...
TEST(SumMatrixOP1, 8) {
  S21Matrix m1({1, 1, -0.7177013520906237});
  S21Matrix m2({1, 1, -8492727.009409374});
  S21Matrix m1_res({1, 1, -8492727.727110725});
...
TEST(SumMatrixOP1, 9) {
  S21Matrix m1({9,
                ...
                50.73194960930169,
                0.44030197929213366});
  S21Matrix m2({1, 2, -1798346.7368360374, -671.5249521564452});

  EXPECT_THROW(m1.SumMatrix(m2), std::invalid_argument);
}

TEST(SumMatrixOP2, 0) {
  S21Matrix m1({1, 2, -5856196548.94238, 28.80287291051794});
  S21Matrix m2({1, 2, 505.64216079961534, 8.81879527614288});
  S21Matrix m1_res({1, 2, -5856196043.30022, 37.62166818666082});
...
TEST(SumMatrixOP2, 4) {
  S21Matrix m1({4, 4, 0.09446135012043366, 83.55731201852635,
